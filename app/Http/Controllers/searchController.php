...
class searchController extends Controller
{
    ...
    {
//        dd($request->all());
        if (!isset($request->agent_name)&&!isset($request->status)&&!isset($request->status)) {
            return redirect()->back()->with("message", "Please Fill up Assigned Agent Name");
        } elseif (!isset($request->priority)) {
            return redirect()->back()->with("message", "Please Fill Up Ticket Priority");
        } elseif (!isset($request->status)) {
            return redirect()->back()->with("message", "Please Fill Up Ticket Status");
        } else {
